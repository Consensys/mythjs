...
var srcmap = require("./srcmap.js")

const { spawn } = require( 'child_process' );

exports.parse_mythril_issue = function(issue) {
...
exports.parse_mythril_output = function(obj, build_obj) {
    ...
    var sourceMap = build_obj.deployedSourceMap
    for (var i in obj.issues) {
	var issue = obj.issues[i]
	var node = srcmap.isVariableDeclaration(issue.address, sourceMap, ast)
	if (node && srcmap.isDynamicArray(node)) {
	    console.log(`skipping issue around dynamic array`)
	} else {
	    parse_mythril_issue(issue)
	}
    }
}
